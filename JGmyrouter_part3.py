    ...
    def router_main(self):    
        ...
        while True:
            ...
            if pkt.has_header(Arp):
                print("Received ARP Routing message")
                arpPkt = pkt[Arp]
                ARPMatched = False
                print("ARP Operation: {}".format(arpPkt.operation))
                print("ARP Reply: {}".format(ArpOperation.Reply))
                if arpPkt.operation==ArpOperation.Reply:
                    print("Responding to ARP Reply")
                    for q in self.queue:
                        print("Items in Queue: {}".format(q))
                        print(arpPkt.senderprotoaddr)
                        print(q.packet[IPv4].dst)
                        if arpPkt.senderprotoaddr == q.packet[IPv4].dst:
                            self.queue.remove(q)
                            print("Buffer data:")
                            for buf in self.buffer:
                                print("Buffer packet: {}".format(buf.packet[IPv4]))
...
#-----------Received ARP packet handling--------------------------------------------
            if pkt.has_header(Arp):
                ...
                arpPkt = pkt[Arp]
                doneReply = False
                print("ARP Packet info: {}".format(arpPkt))
                for interface in self.my_interfaces:
                    print("Checking Interfaces: {} - IP: {}".format(interface.name,interface.ipaddr))
                    print("ARP Target Address: {}".format(arpPkt.targetprotoaddr))
                ...
                if ARPMatched == True:
                    ...
                    if arpPkt.operation==ArpOperation.Request:
                        ...
                        print("Data: TargetEth:{} TargetIP:{} SourceIP:{} SourthEth:{}".format(targetEth,targetIP,sourceIP,sourceEth))
                        arpReply = create_ip_arp_reply(sourceEth,targetEth,sourceIP,targetIP)
                        print("Creating ARP Reply Packet: {}".format(arpReply))
...
#-----------IPv4 packet handling--------------------------------------------
            ...
            if pkt.has_header(IPv4):
                ...
                print("matchEntry Done")
                if matchEntry == 0:
                    print("No match in forwarding table; dropping packet.")
                    continue #if nothing in forwarding table, return to top of loop
                
                #check if ARP table already contains this pair to get mac addr
                print("Displaying ARP Table")
                for x in self.arp_table:
                    print("x: {}|{}".format(x,self.arp_table[x]))
                hasARPAlready = self.checkForAddr(ipPkt)
                print("Result of Checking ARP Table: {}".format(hasARPAlready))
                #pdb.set_trace()
                #if ARP table does have the pair,
                #send the packet out the correct port
                if str(hasARPAlready) != "0":
                    print("HasARP")
                    ipPkt.ttl = ipPkt.ttl - 1
                    outputPort = self.findPort(matchEntry)
                    pkt[Ethernet].dst = hasARPAlready
                    print("Sending Packet: {} out of {}".format(pkt,outputPort))
                    self.net.send_packet(outputPort,pkt)

                #if ARP table does NOT have the pair,
                #create an ARP request and send it. Also enqueue it.
                elif hasARPAlready == 0:
                    print("Does Not have ARP")
                    bufMatch = 0
                    for buf in self.buffer:
                    ...
                    for intf in self.net.interfaces():
                        print(intf.name)
                        print(str(intf.ipaddr))
                        print(str(intf.netmask))
                        print(matchEntry.portName)
                        if matchEntry.prefix == str(intf.ipaddr):
                            srchw = intf.ethaddr
                        elif intf.name==matchEntry.portName:
                            srchw = intf.ethaddr
                    print("Found MAC Address: {}".format(srchw))
                    sendarppkt = create_ip_arp_request(srchw,matchEntry.prefix,ipPkt.dst)
                    print("Sending packet: {}".format(sendarppkt))
    ...
    def insertForwardingTable(self,drmPkt,input_port):
        ...
        numIntf = len(self.my_interfaces)
        while tableLen >= 5+numIntf:
            self.fTable.pop(numIntf)
    ...
    def findPort(self, entry):
        ...
        for port in self.my_interfaces:
            ...
            print("Port IP Addr: {}".format(port.ipaddr))
            print("Entry Prefix: {}".format(entry.prefix))
            if port.ipaddr == IPv4Address(entry.prefix):
                print("Found port: {}".format(port.name))
                return port.name
        return 0

    def checkForAddr(self, entry):
        print("Checking for MAC IP Pair")
        destAddr = IPv4Address(entry.dst)
        print("Destination Address: {}".format(destAddr))
        for x in self.arp_table:
            print("Checking Value in ARP Table: {}".format(x))
            if x == destAddr:      
                print("Final Found Connection: {}".format(self.arp_table[x]))  
    ...
    def checkMatch(self, ipPkt):
        ...
        print("destAddr: {}".format(destAddr))
        for entry in self.fTable:         
            netAddr = IPv4Network(entry.prefix + "/" + entry.mask,strict=False)
            prefixnet = IPv4Network(entry.prefix + "/" + str(netAddr.prefixlen),strict=False)
            destprefix = IPv4Address(destAddr)
            print("Entry: {}".format(entry.prefix))
            print("Net Address: {}".format(netAddr))
            print("Prefixnet: {}".format(prefixnet))
            print("destprefix: {}".format(destprefix))
            if destprefix in prefixnet:
                ...
                if newLength > matchedLength:
                    ...
                    matched = entry
        if matched != 0:
            print("Final Match: {}".format(matched.prefix))
        return matched
