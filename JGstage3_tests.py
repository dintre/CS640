...
def router_tests():
    ...
    #6.  IP packet to be forwarded to 172.0.4.1 should arrive on
    #     router-eth0
    #         Expected event: recv_packet Ethernet
    #         10:00:00:00:00:03->30:00:00:00:00:01 IP | IPv4
    #         192.168.1.100->172.16.42.2 ICMP | ICMP EchoRequest 0 42 (0
    #         data bytes) on router-eth0
    packet = mk_pkt(hwsrc = '10:00:00:00:00:01', hwdst =  '30:00:00:00:00:02', ipsrc  = '192.168.1.100', ipdst = '172.0.4.1')
    s.expect(PacketInputEvent("router-eth0", packet), "IP packet to be forwarded to 172.0.4.1 should arrive on router-eth0")
    
    #7.  Router should send ARP request for 172.0.4.1 out router-
    #     eth0 interface
    #         Expected event: send_packet(s) Ethernet
    #         10:00:00:00:00:03->ff:ff:ff:ff:ff:ff ARP | Arp
    #         10:00:00:00:00:03:172.22.4.1 ff:ff:ff:ff:ff:ff:172.16.42.2
    #         out router-eth2

    arp_request  = create_ip_arp_request('10:00:00:00:00:01', '172.0.0.0', '172.0.4.1')
    s.expect(PacketOutputEvent("router-eth0", arp_request), "Router should send ARP request for 172.0.4.1 out router-eth0 interface")

    # 8. Dynamic message received
    drm_pkt = mk_dynamic_routing_packet('10:00:00:00:00:01',
                                        IPv4Address('172.29.0.0'),
                                        IPv4Address('255.255.0.0'),
                                        IPv4Address('192.168.1.2'))
    print(drm_pkt)
    s.expect(PacketInputEvent("router-eth0", drm_pkt),
